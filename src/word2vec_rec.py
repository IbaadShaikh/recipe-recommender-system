...
def get_recommendations(N, scores):
    """
    Top number of recomendations order by score
    """
...
class TfidfEmbeddingVectorizer(object):
    ...
    def fit(self, docs): 
        ...
        tfidf.fit(text_docs)  # list of text string
...
def get_recs(ingredients, N=5, mean=False):
    ...
    # creating embessing for input text
    input = ingredients
    # creating tokens with elements
    input = input.split(",")
    # parsing ingredient list
    input = ingredient_parser(input)
    # getting embeddings for ingredient
    if mean:
        input_embedding = mean_vec_tr.transform([input])[0].reshape(1, -1)
    else:
        input_embedding = tfidf_vec_tr.transform([input])[0].reshape(1, -1)

    # getting cosine similarity between input embedding and all the document embeddings
    cos_sim = map(lambda x: cosine_similarity(input_embedding, x)[0][0], doc_vec)
    scores = list(cos_sim)
    # Filtering top N recommendations
