...
# Top recomendations order scored
def get_recommendations(N, scores):
    # load in recipe dataset 
    df_recipes = pd.read_csv(config.PARSED_PATH)
    # order the scores with and filter to get the highest number of scores
    top = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[:N]
    #  dataframe to load in recommendations 
...
# cleaning the ingredients being outputted 
